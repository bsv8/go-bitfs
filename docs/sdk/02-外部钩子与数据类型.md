...
// 001、003、004 使用它；005 使用 TransactionEngine 的交易签名校验。
...
```go
// TransactionEngine 负责 MultisigPool/BSV 交易的构造、解析、签名、合并和精确校验。
// 它不访问网络，也不保存状态。
type TransactionEngine interface {
    ...
    SignBuyerPayment(ctx context.Context, payment *pool.UnsignedPayment, buyer Signer) (*pool.PaymentState, error)
    VerifyBuyerPayment(state *pool.PaymentState, proof *pool.OpeningProof) error
    AddSellerSignature(ctx context.Context, state *pool.PaymentState, seller Signer) (*pool.SignedPayment, error)
    AddArbiterSignature(ctx context.Context, state *pool.PaymentState, signature []byte) (*pool.SignedPayment, error)
    BuildImmediateClose(ctx context.Context, input pool.CloseInput) (*pool.UnsignedPayment, error)
...
type OpeningInput struct {
    ...
    ExpiryLockTime  uint32 // 费用池约定的区块高度或 Unix 时间锁。
    SellerPubKey    []byte // 卖方公钥。
    ArbiterPubKey   []byte // 本池唯一仲裁公钥。
}

// PaymentUpdateInput 是 pool 层的低层输入，适用于非 BitFS 的通用费用池调用。
// buyer.Client 不直接暴露 SellerAmountAfter；它会从已验证的 001、003、004 自动计算。
type PaymentUpdateInput struct {
    ...
    MinerFeeSat          uint64        // 从买方找零扣除的交易费。
}
