...
1. `wire` 通过类型化 encoder 与严格 decoder 为全部十一种 Kind 返回不可变的 exact-bytes Artifact；001/003/004 位于 `content`，002/005/006 位于 `pool`，007/008 托管证据位于 `arbitration`——确定性 CBOR、严格解码与证据验证均已完成。
...
3. 角色 workflow（`buyer`、`seller`、`arbiter`）只持有构造时固定的受约束 Signer（本地软件私钥的唯一入口是 `protocol.NewPrivateKeySigner`）。每个方法都显式接收业务输入（已验收报价、池 checkpoint、交付上下文、内容字节、seed）加一份显式 Facts，只返回计算得到的 Artifact、包装为 verified 值的交易原文、已验证证据以及需要应用自行持久化的 opaque checkpoint。
