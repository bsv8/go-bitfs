...
go-bitfs 是 001–008 的可执行协议规范，同时是一个**无状态、无基础设施副作用的协议 SDK**：角色 workflow 只持有传入 `WorkflowConfig{PrivateKey}` 的官方 BSV 私钥，对显式传入的输入执行确定性的 Build/Verify/Sign/Merge 计算。
