...
| 文档 | 说明 |
|---|---|
| [01 · 协议基础与 CBOR](01-协议基础与CBOR.md) | 包边界、通用约定、错误模型、统一 `wire` 编解码与纯协议函数。 |
| [02 · 外部钩子与数据类型](02-外部钩子与数据类型.md) | 签名、存储、内容、交易引擎、BSV 节点钩子及关键输入类型。 |
| [03 · 角色工作流 API](03-角色工作流API.md) | 买方、卖方、仲裁者 API，以及最短端到端调用顺序。 |
| [04 · 实施路线](04-实施路线.md) | 实施先后顺序与必须覆盖的验证场景。 |
