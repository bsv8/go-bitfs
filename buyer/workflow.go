...
// exact ClaimCBOR and Claim ID from its own OpeningProof plus the exact signed
// 003 — no Seller Claim signature, payload, or Kind 9 is needed — signs
// [4,10,claim_id,nonce] with this workflow's private key, self-verifies the
...
func (workflow *Workflow) BuildArbitrationContentRequest(ctx context.Context, opening *pool.OpeningProof, authorization *bitfs.SignedContentRequest, nonce []byte) (*arbitration.ContentRetrievalRequest, error) {
	...
	signing, err := arbitration.BuyerRetrievalSigningCBOR(built.ClaimID, localNonce)
	if err != nil {
		return nil, err
	}
	signature, err := bitfs.SignMessage(workflow.privateKey, signing)
	if err != nil {
		return nil, err
	}
	if err := bitfs.VerifySignature(workflow.publicKey, signing, signature); err != nil {
		return nil, fmt.Errorf("%w: generated buyer retrieval signature failed verification: %v", pool.ErrInvalidEvidence, err)
	}
	request := &arbitration.ContentRetrievalRequest{Version: arbitration.MajorVersion, ClaimID: append([]byte(nil), built.ClaimID...), Nonce: append([]byte(nil), localNonce...), BuyerSignature: append([]byte(nil), signature...)}
	if _, err := arbitration.MarshalContentRetrievalRequest(request); err != nil {
		return nil, err
	}
	return request, nil
}
...
type VerifiedArbitratedContent struct {
	// ClaimID is the verified arbitration claim identity of the custody record.
	ClaimID []byte
	// Payloads holds the verified content bytes in authorized order.
	Payloads [][]byte
	// Receipt is the verified Kind 9 receipt for offline audit.
...
// time-independent quote/003/opening evidence, rebuilds the local expected
// ClaimCBOR from opening + exact 003, verifies the exact Kind 10 request and
// its buyer signature, strict-decodes both embedded documents, requires the
// embedded ClaimCBOR to equal the locally rebuilt one byte for byte, runs the
// full arbitration custody verification chain (Seller Claim, Receipt Claim ID,
// Arbiter receipt signature, Arbiter transaction signature), then re-checks
// payload membership, protocol lengths, aggregate pricing, and the previous
// state continuity. Expired quotes, deadlines, or refunds never reject
// already signed custody evidence.
func (workflow *Workflow) AcceptArbitratedContent(ctx context.Context, quote *bitfs.SignedFileQuote, opening *pool.OpeningProof, previous *pool.PaymentState, authorization *bitfs.SignedContentRequest, retrievalRequest *arbitration.ContentRetrievalRequest, retrievalResponse *arbitration.ContentRetrievalResponse, input ArbitratedContentInput) (*VerifiedArbitratedContent, error) {
	...
	// 精确验证自己发出的 Kind 10：Claim ID、nonce 与买方签名（全部使用快照）。
	signing, err := arbitration.BuyerRetrievalSigningCBOR(localRetrievalRequest.ClaimID, localRetrievalRequest.Nonce)
	if err != nil {
		return nil, err
	}
	if err := bitfs.VerifySignature(workflow.publicKey, signing, localRetrievalRequest.BuyerSignature); err != nil {
		return nil, fmt.Errorf("%w: buyer retrieval signature invalid: %v", bitfs.ErrInvalidEvidence, err)
	}
	if !bytes.Equal(localRetrievalRequest.ClaimID, built.ClaimID) {
		return nil, fmt.Errorf("%w: retrieval request does not reference the locally rebuilt claim", bitfs.ErrInvalidEvidence)
	}
	// strict decode 内嵌的 exact Kind 8/9（decoder 同时强制 canonical）。
	embeddedRequest, err := arbitration.UnmarshalRequest(localRetrievalResponse.ArbitrationRequestCBOR)
	if err != nil {
		return nil, err
	}
	embeddedResponse, err := arbitration.UnmarshalResponse(localRetrievalResponse.ArbitrationResponseCBOR)
	if err != nil {
		return nil, err
	}
	// 内嵌 ClaimCBOR 必须与本地期望逐字节相等，不只比较 hash。
	if !bytes.Equal(embeddedRequest.ClaimCBOR, built.ClaimCBOR) {
		return nil, fmt.Errorf("%w: embedded claim differs from the locally rebuilt claim", bitfs.ErrInvalidEvidence)
	}
	// 完整托管证据链：Seller Claim、回执 Claim ID、回执签名、交易签名。
	verified, err := arbitration.VerifyCustodiedContent(embeddedRequest, embeddedResponse)
	if err != nil {
		return nil, err
	}
	contentHashes, err := bitfs.DecodeContentHashes(requestTerms.ContentHashesCBOR)
	...
	effectiveSeed, err := bitfs.VerifyContentPayloadsContext(ctx, quoteTerms, contentHashes, verified.Payloads, localSeed)
	if err != nil {
		return nil, err
	}
	price, err := bitfs.ContentHashesPriceSat(quoteTerms, contentHashes, effectiveSeed)
	...
	receipt, err := arbitration.UnmarshalReceipt(embeddedResponse.ReceiptCBOR)
	if err != nil {
		return nil, err
	}
	return &VerifiedArbitratedContent{ClaimID: append([]byte(nil), verified.ClaimID...), Payloads: clonePayloads(verified.Payloads), Receipt: receipt}, nil
}
...
func cloneRetrievalRequest(request *arbitration.ContentRetrievalRequest) *arbitration.ContentRetrievalRequest {
	...
	return &arbitration.ContentRetrievalRequest{Version: request.Version, ClaimID: append([]byte(nil), request.ClaimID...), Nonce: append([]byte(nil), request.Nonce...), BuyerSignature: append([]byte(nil), request.BuyerSignature...)}
}

// cloneRetrievalResponse 深拷贝一份 Kind 11 输入快照，包括两份内嵌文档。
func cloneRetrievalResponse(response *arbitration.ContentRetrievalResponse) *arbitration.ContentRetrievalResponse {
	if response == nil {
		return nil
	}
	return &arbitration.ContentRetrievalResponse{Version: response.Version, ArbitrationRequestCBOR: append([]byte(nil), response.ArbitrationRequestCBOR...), ArbitrationResponseCBOR: append([]byte(nil), response.ArbitrationResponseCBOR...)}
}
