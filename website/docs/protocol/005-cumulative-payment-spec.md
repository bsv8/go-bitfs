...
When submission fails, times out, or the txid/sequence number is inconsistent, the local accepted state MUST NOT advance; if the node has accepted the transaction but local persistence fails, the system MUST enter the `external-state-uncertain` state.
