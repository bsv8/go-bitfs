...
//
// fixture 先完成一轮交付（003→004，不执行普通付款）；卖方角色 API 从本地
// 开池证据、买方授权与本方已发交付构造 exact Kind 8 Artifact；仲裁方角色 API
// 先 PrepareArbitration 完整验证（应用在此持久化托管证据），再独立重建并签署
// Kind 9；最后卖方 CompleteArbitratedPayment 验证回执、补签并合并完整交易。
// 是否广播由应用决定。
package main
...
func main() {
	...
	kind8Artifact, err := f.Seller.PrepareArbitration(ctx, f.Facts(now), seller.ArbitrationCommand{
		Pool:        f.SellerPool,
	...
	// exact 托管证据（RequestCBOR、Claim ID、冻结费用与 payload bundle）。
	prepared, err := f.Arbiter.PrepareArbitration(f.Facts(now), rawKind8, fee)
	if err != nil {
		fail(fmt.Errorf("arbiter.PrepareArbitration: %w", err))
	}
	custodyClaimID := prepared.ArbitrationClaimID()
	exactRequestCBOR, err := prepared.RequestCBOR()
	if err != nil {
		fail(err)
	}
	...
	custody := map[string]*custodyRecord{custodyKey: {
		RequestBytes:          append([]byte(nil), exactRequestCBOR...),
	...
	rawKind9 := response9Artifact.Bytes()
	// 签名后只把 exact canonical Kind 9 追加到原记录，绝不整体覆盖。
	saved.ResponseBytes = append([]byte(nil), rawKind9...)
	persisted := custody[custodyKey]
	if string(persisted.ResponseBytes) != string(rawKind9) || string(persisted.RequestBytes) != string(exactRequestCBOR) {
	...
	arbitratedTx, err := f.Seller.CompleteArbitratedPayment(ctx, f.Facts(now), seller.ArbitratedPaymentCommand{
		RequestRaw:           rawKind8,
