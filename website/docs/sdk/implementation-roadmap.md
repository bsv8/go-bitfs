...
2. MultisigPool is the sole implementation of payment-pool transactions, `SIGHASH_ALL|FORKID`, 2-of-3 scripts, cumulative payments, final close, and refund-expiry checks. Role workflows internally wrap the caller's narrow `NonFinalPoolBackend` or `PoolBackend` in `pool.VerifiedNonFinalPoolNode`.
...
5. The current surface contains only BitFS v4 role workflows and `pool` role adapters. The former `settlement`/runtime dual stack has been removed; there is no legacy build-tag acceptance path.
