...
The outer version `4` does not enter the buyer signature. There is no inner version field. The terms carry no public keys and no fee rate; identity and fees are recovered from the OpeningProof bound to `refund_template_txid`. The authorization hash serves as the `PaymentAuthorizationHash` for subsequent 004, 005, and 007. The current version does not carry a negotiable arbitration amount; the construction layer always uses `ArbiterAmount = 0`.
