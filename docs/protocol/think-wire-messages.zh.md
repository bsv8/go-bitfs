# BitFS Wire Protocol v1 统一报文设计稿

> 状态：设计讨论稿，不是当前实现真值。
>
> 本文只重新设计 wire 的版本模型、报文外形、认证文档、签名域、ID 与命名规则。
...
```text
file_quote_terms_cbor          -> file_quote_terms_id
payment_authorization_cbor     -> payment_authorization_id
content_delivery_cbor          -> content_delivery_id
arbitration_claim_cbor         -> arbitration_claim_id
arbitration_receipt_cbor       -> arbitration_receipt_id
content_retrieval_request_cbor -> content_retrieval_request_id
content_retrieval_result_cbor  -> content_retrieval_result_id
content_payloads_cbor          -> content_payloads_id
```
...
现有 `ContentRequestTerms` 的实际协议语义是“Buyer 的最终付款授权”，因此统一命名为
...
原 `quote_terms_hash` 统一改名为 `file_quote_terms_id`；原
`payment_authorization_hash` 统一改名为 `payment_authorization_id`。
两者的算法仍然是 SHA-256，只改变协议对象命名，不改变业务关联关系。
...
content_delivery_id = SHA-256(content_delivery_cbor)
...
这只把原来 Seller 对裸 `payment_authorization_hash` 的签名改成通过统一 Kind 6
签名上下文认证 `content_delivery_cbor`，不改变交付内容和付款授权的对应关系。
...
arbitration_receipt_id = SHA-256(arbitration_receipt_cbor)
...
content_retrieval_result_id =
  SHA-256(content_retrieval_result_cbor)
...
现有 `spec/v1` 和 `spec/v3` 属于未上线的历史迭代，不应继续占用现行版本目录。
落地时应移动到 `spec/legacy/` 或删除；新的 `spec/v1/` 是唯一现行 CDDL 真值。
...
现规则要求同一 `(arbitration_claim_id, retrieval_nonce)` 只有一个请求成功，超时必须换
nonce。这会使网络重试不幂等，也可能让先到达的重放请求消耗 nonce。

建议应用层考虑：

- exact `content_retrieval_request_id` 重放返回同一份已持久化响应；
- 同 nonce 搭配不同请求文档才判定为冲突；
- 为请求增加应用层短时有效期，避免 nonce 记录永久保存；
- 如果未来取件会扣费或产生一次性副作用，再升级为 Arbiter 发放 server challenge。

这些变化涉及重试、计费和存储语义，因此不在本结构稿中直接加入 `expires_at` 或 server nonce。
