...
同一个池不能同时被卖方接受为多张未完成请求，否则同一笔余额会被重复承诺，买方可能在卖方交付多个内容后只支付其中一笔。这是调用方应用的本地运行约束；它不把数据库变成真值，卖方保存的已签名支付状态才是依据。SDK 仅验证显式输入；交付上下文的串行化、持久化与确认时机见 005 中对调用方应用职责的说明。
...
具体字段和校验规则见[内容获取请求规范](003-content-request-spec.md)。
