...
BitFS v3 的仲裁提交只执行卖方提供、且可验证的最终付款授权与候选状态：
...
买方到期退款的矿工费由退款交易既有规则承担。仲裁者的商业服务费在 BitFS v3 由卖方在池外支付或由卖方为提交交易额外提供输入承担；一旦要从池内扣除，就属于仲裁金额设计，必须另行规范。
...
具体关闭报文和 BitFS v3 的未定边界见[费用池无条件关闭规范](006-unconditional-pool-close-spec.md)。
